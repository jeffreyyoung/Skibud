...
var NavigationBarRouteMapper = { 
  LeftButton: function( route, navigator, index, navState ){
    ...
    if (index) {
      return(
        <TouchableOpacity onPress={() => navigator.pop()} style={[{flexDirection: 'row'}, styles.leftButton]}><FIcon style={styles.navBarText} name='chevron-left' size={8} color='white'/><Text style={styles.navBarText}></Text></TouchableOpacity>
      )
    } else {
      return null;
    }
  ...
  Title: function( route, navigator, index, navState ){
    return(
      <Text style={[styles.navBarText, styles.navBarTitleText]}>{route.scene}</Text>
    )
  },
